(async function () {
  ...
  function render() {
    ...
    if (filtered.length === 0) {
      ...
      if (allReports.length === 0) {
        ...
        emptyEl.textContent = "No reports match the search.";
      }
  ...
  function verdictBadge(rawInvestigation) {
    ...
    span.textContent = investigation.verdict;
    if (typeof investigation.confidence === "number") {
      const conf = document.createElement("span");
      conf.className = "bon-verdict-conf";
      conf.textContent = `${Math.round(investigation.confidence * 100)}%`;
  ...
  function renderFactorDots(investigation) {
    ...
    for (const key of FACTOR_KEYS) {
      ...
      const leaning = f ? scoreLeaning(f.score, f.confidence) : "missing";
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (res?.ok === false && res.error === "no-api-key") {
          alert(
            "No Claude API key set. Open the toolbar popup and add a key in Settings."
          );
        }
  ...
  function renderInvestigationDetail(rawInvestigation) {
    ...
    if (Array.isArray(investigation.factors) && investigation.factors.length) {
      ...
      const ordered = [
        ...FACTOR_KEYS.filter((k) => byKey.has(k)).map((k) => byKey.get(k)),
        ...investigation.factors.filter((f) => !FACTOR_KEYS.includes(f.key)),
      ];
    ...
    if (typeof f.score === "number") {
      ...
      pill.textContent =
        leaning === "neutral" || leaning === "missing"
          ? "neutral"
          : `${leaning} ${Math.abs(f.score).toFixed(2)}`;
      header.appendChild(pill);
    }
    li.appendChild(header);

    if (typeof f.score === "number") {
      li.appendChild(renderScoreBar(f.score, f.confidence));
    }
