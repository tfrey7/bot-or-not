...
import { bonInferRegion } from "../regions/index.js";
import { bonIsInvestigationStale } from "../../verdict.js";
import { BON_REPORTS_VERDICT_RANK } from "./data.js";
...
export function bonReportsExpectedDurationMs(allReports) {
  const durs = [];
...
export function bonReportsFormatExpectedSec(ms) {
  return Math.max(1, Math.round(ms / 1000));
}

export function bonReportsFormatRunningCellText(elapsedSec, expectedMs) {
  if (!expectedMs) {
...
export function bonReportsFormatRunningTitle(elapsedSec, expectedMs) {
  if (!expectedMs) {
...
export function bonReportsIsActivityFresh(activityData) {
  return (
...
export function bonReportsSanitizeUsernameQuery(raw) {
...
export function bonReportsDiagnoseLoadError(message) {
...
export function bonReportsHasStructuralChange(prev, next) {
...
export function bonReportsInferTimezoneFromTimestamps(timestamps) {
...
export function bonReportsComputeRegionForReport(report) {
...
export function bonReportsComputeEarliestFullyVisible(activityData) {
...
export function bonReportsDefaultDirFor(key) {
...
export function bonReportsSortValue(r, key, regionLabels) {
  if (key === "username") {
  ...
  if (key === "verdict") {
    ...
    return BON_REPORTS_VERDICT_RANK[v] ?? 5;
  }
...
export function bonReportsCompareBy(key, dir, regionLabels) {
