...
export function bonFmtDuration(ms) {
...
export function bonFormatDate(ts) {
  const d = new Date(ts);
...
export function bonFormatPanelDate(ts) {
  const diffMs = Date.now() - ts;
  const min = 60_000;
  const hour = 60 * min;
  const day = 24 * hour;
...
export function bonFmtTimestamp(ts) {
  const d = new Date(ts);
