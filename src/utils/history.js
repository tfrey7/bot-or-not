...
export function bonMergeHistoryEntries(a, b) {
...
export function bonDedupeHistory(history) {
  const seen = new Map();
  const out = [];
...
export function bonNormalizeReport(value) {
...
export function bonFindReportKey(reports, username) {
  if (reports[username]) {
    return username;
  }
...
export function bonSnapshotRun(inv, status) {
  return {
